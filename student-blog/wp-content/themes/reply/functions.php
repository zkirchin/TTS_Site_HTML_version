...
if ( !function_exists( 'mighty_theme_setup' ) ) {
    function mighty_theme_setup() {
        ...
        add_image_size( 's', 430, '', true );
        add_image_size( 'm', 820, '', true );
...
if ( !function_exists( 'mighty_enqueue_scripts' ) ) {
	function mighty_enqueue_scripts() {
		...
		wp_enqueue_style( 'googlefont-opensans', $protocol . '://fonts.googleapis.com/css?family=Open+Sans:400italic,400,600,700,800' );
...
if ( ! function_exists( 'mighty_article_nav' ) ) :
	...
	function mighty_article_nav() {
		...
		<nav id="article-nav" class="clearfix" role="navigation">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">Previous Post</span> %title', 'Previous post link', 'mighty' ) );
				next_post_link( '<div class="nav-next">%link</div>', _x( '<span class="meta-nav">Next Post</span> %title', 'Next post link', 'mighty' ) );
			?>
		</nav>
